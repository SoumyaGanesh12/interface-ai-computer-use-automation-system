...
**4. Compile an existing trace into a reusable capability**
...
**5. Browse and invoke capabilities the way an agent would**
...
- **Allowlist, narrowed by deployment config.** An artifact's own `policy` is checked at
  ...
  (`config/allowlist.json`) — an artifact can narrow what it touches, never widen past
